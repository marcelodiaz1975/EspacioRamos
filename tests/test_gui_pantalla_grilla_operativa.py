...
def _unidad_con_consultorio(conn, nombre_edificio, departamento, numero=1, valor_regular=1000, valor_aislada=500):
    id_edificio = obtener_repositorio(conn, "Edificio").crear(Nombre=nombre_edificio, DomicilioLocalidad="Ramos Mejía")
...
def test_estadisticas_incluyen_unidad_edificio_y_total(qtbot, conn):
    ...
    assert pantalla.tabla_estadisticas.rowCount() == 3  # unidad + edificio + total
    nombres = [pantalla.tabla_estadisticas.item(f, 0).text() for f in range(3)]
    assert nombres[0] == 'Ramos 1 - 7mo "L"'
    assert nombres[1] == "Ramos 1"
    assert nombres[2] == "Total"
...
def test_valores_y_estadisticas_siguen_el_filtro_de_unidad(qtbot, conn):
    ...
    assert pantalla.tabla_valores.rowCount() == 1
    assert pantalla.tabla_valores.item(0, 0).text() == "Ramos 1"
    assert pantalla.tabla_estadisticas.rowCount() == 3  # unidad + subtotal de su edificio + total
...
def test_subtotal_regulares_coincide_con_el_motor_de_estadisticas(qtbot, conn):
    ...
    esperado = calcular_estadisticas_operativas(conn, [id_unidad]).por_unidad[0]
