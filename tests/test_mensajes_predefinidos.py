...
from app.db.seed import CATEGORIA_MENSAJES_SITUACIONES, sembrar_valores_por_defecto
...
def test_combo_filtro_incluye_todas_y_las_categorias_sembradas(qtbot, conn):
    ...
    assert CATEGORIA_MENSAJES_SITUACIONES in categorias
...
def test_filtrar_por_categoria_oculta_las_demas_filas(qtbot, conn):
    ...
    indice = pantalla.combo_filtro.findData(CATEGORIA_MENSAJES_SITUACIONES)
    ...
    for oculta, categoria in zip(ocultas, categorias_filas):
        assert oculta == (categoria != CATEGORIA_MENSAJES_SITUACIONES)
